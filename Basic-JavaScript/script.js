...
/*
Variables wich are declared within a function, as well as the function parameters,
...
These conditions are known as boolean conditions and the may only be true or false.
...
When test is called with a value of true, the if statement evaluates myCondition to see if it is true
...
/*
The less than operator (<) compares the values of two numbers. If the number to the left
...
Liek the equality operator, the less than or equal operator converts data types.
...
/*
Sometimes you will need to test more than on thing at a time.
The logical and operator (&&) returns true if and only if the operands to the left and
